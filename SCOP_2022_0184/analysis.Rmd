...
# get gene names for each GO-term
ontology_gene_names <- lapply(GO_terms$index,
                              plyr::mapvalues,
                              from = names(gene_names),
                              to=gene_names, 
                              warn_missing=F)


ontology_gene_names_df <- ontology_gene_names %>% 
  lapply(paste, collapse=", ") %>% 
  unlist() %>% 
  enframe(name = "ID", value = "Genes")

for (contrast in colnames(contrast_matrix)) {
  ontology_tests[[contrast]] %>% 
    left_join(ontology_gene_names_df) %>% 
...
The figures below show the location of the samples in reduced 2-dimensional space. Again, we see that the samples seem to cluster somewhat into two distinct groups.
...
ggplot_mds(y = y, 
           ...
           size=1) +
  ggtitle("MDS-plot colored by gRNA (Dimensions 1 & 2)") + 
  aes(label = rownames(y$samples)) +
  geom_label_repel()
```
...
## The investigated *contrasts* and a short summary of the results

Throughout the analysis-section, you will see tables containing the differential gene expression analysis results as well as the gene-ontology enrichment results for the different contrasts, or groupings, as outlined just below. I have also added a small summary of the results for each contrast.

1.  PHOX2B vs EV with proliferation media (PHOX2B_vs_EV_proliferation)
    + Number of significantly differentially expressed genes: `r I(all_genes_test_full_info[["PHOX2B_vs_EV_proliferation"]] %>% filter(FDR<0.05) %>% nrow())`
    + Number of significantly enriched GO-terms: `r I(ontology_tests[["PHOX2B_vs_EV_proliferation"]] %>% filter(FDR<0.05) %>% nrow())`
2.  PHOX2B vs EV with differentiation media (PHOX2B_vs_EV_differentiation)
    + Number of significantly differentially expressed genes: `r I(all_genes_test_full_info[["PHOX2B_vs_EV_differentiation"]] %>% filter(FDR<0.05) %>% nrow())`
    + Number of significantly enriched GO-terms: `r I(ontology_tests[["PHOX2B_vs_EV_differentiation"]] %>% filter(FDR<0.05) %>% nrow())`
3.  PTPRK vs EV with proliferation media (PTPRK_vs_EV_proliferation)
    + Number of significantly differentially expressed genes: `r I(all_genes_test_full_info[["PTPRK_vs_EV_proliferation"]] %>% filter(FDR<0.05) %>% nrow())`
    + Number of significantly enriched GO-terms: `r I(ontology_tests[["PTPRK_vs_EV_proliferation"]] %>% filter(FDR<0.05) %>% nrow())`
4.  PTPRK vs EV with differentiation media (PTPRK_vs_EV_differentiation)
    + Number of significantly differentially expressed genes: `r I(all_genes_test_full_info[["PTPRK_vs_EV_differentiation"]] %>% filter(FDR<0.05) %>% nrow())`
    + Number of significantly enriched GO-terms: `r I(ontology_tests[["PTPRK_vs_EV_differentiation"]] %>% filter(FDR<0.05) %>% nrow())`
