...
meta_data_sheet <- readxl::read_excel("0177_metadata.xlsx") %>%
  transmute(`Sample.ID`=sub(pattern = ".....", replacement = "", x=`Sample ID (Library ID)`), 
            ...
            Replicate=factor(gsub(`Cage no. (1,2,3)`, pattern="replicate_", replacement = ""), levels=c("1","2","3")))%>%
  as.data.frame()
...
contrast_matrix <- makeContrasts(PHOX2B_vs_EV_differentiation=groupPHOX2Bdifferentiation-groupEVdifferentiation,
                                 levels=design)
...
```{r}
annotation_col_df <-  dplyr::select(meta_data_sheet, gRNA, Extraction,Media)
...
plot_sample_heatmap(log_cpm,
                    ...
                    annotation_col = annotation_col_df[c("gRNA","Extraction","Media")],
                    display_numbers=T,
                    fontsize_number=6,
...
ggplot_mds(y = y, 
           dim_plot = c(1,2),
           colour_by = y$samples$Extraction,
           size=1) +
  ggtitle("MDS-plot colored by gRNA") + 
  aes(label = rownames(y$samples)) +
  geom_label_repel()
